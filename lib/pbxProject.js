...
pbxProject.prototype.addPluginFile = function(path, opt) {
    ...
    if(opt.group) this.addToPbxGroup(opt.group, file);
    else this.addToPluginsPbxGroup(file);            // PBXGroup
...
pbxProject.prototype.addResourceFile = function(path, opt) {
    ...
    file.target = opt ? opt.target : undefined;
    if (!opt.plugin) {
        if(opt.group) correctForPath(file,this,opt.group);
    ...
    if (!opt.plugin) {
        if(opt.group != "Resources") file.group = opt.group;
        this.addToPbxFileReferenceSection(file);    // PBXFileReference
...
pbxProject.prototype.addPbxGroup = function(filePathsArray, name, path, sourceTree) {
    ...
    if (groups) {
        ...
        // add group to product group
        var parentGroup = this.getFirstProject().firstProject.mainGroup;;
        if(groups[parentGroup]!=undefined) groups[parentGroup].children.push(
            pbxGroupChild({fileRef:pbxGroupUuid, basename: name}));
...
pbxProject.prototype.addToPbxGroup = function(groupName, file) {
...
pbxProject.prototype.addToResourcesPbxGroup = function(file) {
    var groupName = (file.group!=undefined)?(file.group):"Resources";
    var groupObject = this.pbxGroupByName(groupName);
    groupObject.children.push(pbxGroupChild(file));
}
...
pbxProject.prototype.addTargetDependency = function(target, dependencyTargets) {
    ...
    for (var index = 0; index < dependencyTargets.length; index++) {
        ...
        if (!pbxContainerItemProxySection || pbxContainerItemProxySection==undefined) {
            //this.hash.project.objects[pbxContainerItemProxy] = {"comment":"PBXContainerItemProxy Section"};
            this.hash.project.objects[pbxContainerItemProxy] = {};
            pbxContainerItemProxySection = this.hash.project.objects[pbxContainerItemProxy];
        }
        if (!pbxTargetDependencySection || pbxTargetDependencySection==undefined){
            //this.hash.project.objects[pbxTargetDependency] = {"comment": "PBXTargetDependency Section"};
...
function correctForPath(file, project, group) {
    var r_group_dir = new RegExp('^' + group + '[\\\\/]'),
        groupObj = project.pbxGroupByName(group);

    if(groupObj) if(groupObj.path) file.path = file.path.replace(r_group_dir, '');
    return file;
}
